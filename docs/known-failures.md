...
| 6b | Small mid-line clef change not recognised at all (the staff is read in the old clef) | Recognition | Little Baby in a Manger m14–16 (piano left hand a 6th low) | 10 chords | Medium / low | Manual fix available: `--clef MEASURE:PART:STAFF:CLEF` (e.g. `--clef 14:Piano:2:treble`) re-reads the staff. Automatic detection is still open. | Hard to detect in general |
